...
Roamnio is an AI travel concierge that begins with the traveller, not the destination — before showing any options, it asks a short set of questions about who's travelling, physical capability, dietary needs, pace, and budget, capturing each group member's individual needs (children, adults, elderly) within a single shared trip profile so the plan works for everyone travelling, not just the person filling in the form. 

That profile drives a full day-by-day itinerary generated by AI and shown as interactive cards instead of a block of text, which the user can regenerate day-by-day, pick hotels from, and see physical-difficulty warnings tailored to each traveller's stated limits.
